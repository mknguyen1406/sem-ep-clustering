...
import org.streampipes.examples.jvm.processor.birch.net.sourceforge.sizeof.*;
...
public class CFTree {
	 ...
	 */
	public CFTree(int maxNodeEntries, double distThreshold) {

		int distFunction = D0_DIST;
			
		root = new CFNode(maxNodeEntries,distThreshold,distFunction,true);
		leafListStart = new CFNode(0,0,distFunction,true); // this is a dummy node that points to the fist leaf
	...
	private boolean rebuildIfAboveMemLimit() {
		if(hasReachedMemoryLimit(this,memLimit)) {
			...
			CFTree newTree = this.rebuildTree(root.getMaxNodeEntries(), newThreshold, root.getDistFunction(), false);
			copyTree(newTree);
	...
	private void splitRoot() {
		...
		CFNode newNode1 = new CFNode(root.getMaxNodeEntries(),root.getDistThreshold(),root.getDistFunction(),root.isLeaf());
		newEntry1.setChild(newNode1);
		
		CFEntry newEntry2 = new CFEntry();
		CFNode newNode2 = new CFNode(root.getMaxNodeEntries(),root.getDistThreshold(),root.getDistFunction(),root.isLeaf());
		newEntry2.setChild(newNode2);
		
		// the new root that hosts the new entries
		CFNode newRoot = new CFNode(root.getMaxNodeEntries(),root.getDistThreshold(),root.getDistFunction(),false);
	 ...
	 */
	public CFTree rebuildTree(int newMaxEntries, double newThreshold, int distFunction, boolean discardOldTree) {
		CFTree newTree = new CFTree(newMaxEntries, newThreshold);
	...
	/**
	 * Continues inserting data and therefore resets the SubClusterIds of all leaves
	 * The obtained subclusters will be assigned a positive, unique ID number
	 */
	public void continueInsertingData() {
	...
	/**
	 * Returns all leaf entries
	 * @return
	 */
	public List<CFEntry> getLeafEntries() {

		List<CFEntry> leafEntries = new ArrayList<CFEntry>();
